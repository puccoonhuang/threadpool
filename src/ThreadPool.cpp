...
{
public:
	PooledThread(const std::string& name, int stackSize = TP_THREAD_STACK_SIZE);
	~PooledThread();
...
{
	TP_assert (minCapacity >= 1 && maxCapacity >= minCapacity && idleTime > 0);
...
void ThreadPool::addCapacity(int n)
{
	...
	TP_assert (_maxCapacity + n >= _minCapacity);
	_maxCapacity += n;
	housekeep();
}
...
public:
	...
	ThreadPool* pool()
	{
		...
		if (!_pPool)
		{
			...
			if (TP_THREAD_STACK_SIZE > 0)
				_pPool->setStackSize(TP_THREAD_STACK_SIZE);
		}
		return _pPool;
	}
